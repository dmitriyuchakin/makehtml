...
class DocxToHtmlConverter:
    ...
    def _process_runs(self, paragraph: Paragraph) -> str:
        ...
        for run in paragraph.runs:
            ...
            run_hyperlink = None
            for link_id, link_url, link_runs in hyperlinks:
                if run._element in link_runs:
                    run_hyperlink = (link_id, link_url)
                    break
            ...
            if run_hyperlink and run_hyperlink != current_hyperlink:
                ...
                if current_hyperlink and hyperlink_text:
                    combined_text = ''.join(hyperlink_text)
                    result.append(f'<a href="{current_hyperlink[1]}">{combined_text}</a>')
                    hyperlink_text = []
    ...
    def _extract_hyperlinks(self, paragraph: Paragraph) -> List[tuple]:
        ...
        for idx, hyperlink in enumerate(hyperlink_elements):
            ...
            if url:
                ...
                hyperlinks.append((link_id, url, run_elements))
            ...
            if fld_char is not None and fld_char.get('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}fldCharType') == 'begin':
                ...
                if url:
                    ...
                    if link_runs:
                        ...
                        hyperlinks.append((link_id, url, link_runs))
                        field_idx += 1
...
def get_app_config_dir() -> Path:
    ...
    app_support = Path.home() / 'Library' / 'Application Support' / 'DOCX2HTML'
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    2. Otherwise, check ~/Library/Application Support/DOCX2HTML/config.json
